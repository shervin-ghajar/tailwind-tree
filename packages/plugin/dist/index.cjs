    ...
    switch (node.type) {
        ...
        case 'TemplateLiteral':
            if (node.expressions.length === 0)
                return [prefix + node.quasis.map((q) => q.value.cooked).join('')];
            return [];
...
const collectUsedClasses = (sourceFiles) => {
    ...
    sourceFiles.forEach((filePath) => {
        ...
        try {
            ...
            console.log({ sourceFiles, content });
        }
        catch {
            console.warn(`⚠️  Failed to read file: ${filePath}`);
            return;
        }
        if (!/\btwTree\s*\(/.test(content))
            return;
...
const writeSafelistFile = (classes, safelistPath) => {
    const filePath = path.resolve(__dirname$1, 'safelist.css');
...
const generateTwSafelist = async (filePaths, safelistPath) => {
    try {
        const filePath = safelistPath ?? path.resolve(__dirname$1, 'safelist.css');
        // 1. If safelist itself is passed as file, skip
        if (filePaths?.some((f) => path.resolve(f) === path.resolve(filePath)))
            return;
        ...
        if (fs.existsSync(filePath)) {
            ...
            console.log({ classes, safelist: content.match(/@source inline\("([^"]+)"\);/g) });
            existingClasses = Array.from(new Set(content
...
exports.twTreePlugin = twTreePlugin;
