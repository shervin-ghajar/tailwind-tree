...
const ROOT_PATH = path.resolve(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))), "../"); //dist path
...
const checkTwSafelistFileExists = (filePath) => {
    ...
    child_process.execSync("npx generate-rn-stylewind");
};
...
exports.getTwSafelist = getTwSafelist;
